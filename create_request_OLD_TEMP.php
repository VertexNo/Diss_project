...
fk_create_user_id, fk_responsible_user_id, date_create, fk_status_id, fk_priority_id, fk_service_id) values(null,'" . $caption . "', '" . $short_description . "', '" . $description ."' , '".$userCreate . "', '" . $resultUserResponsID ."','".$dateCreate. "', '" . $statusid . "', '" . $resultPriorityID . "', '" . $resultServiceID . "' )");
        ...
        if (!$result_query_insert) {
            ...
            /*Начало логики по прикрплению файлов*/
...
// Директория куда будут загружаться файлы.
            ...
            if (isset($_FILES[$input_name])) {
                ...
                foreach ($files as $file) {
                    ...
                    if (!empty($success)) {
                        //$CurrentInsertRequestID
                        //Здесь загрузка в базу
                        /*$result_query_insertAttachments = $mysqli->query("insert into Attachments (attachment_id,attachment_url, fk_request_id)
values(null,'".$path.$file["file"]["name"] . "', '" . $CurrentInsertRequestID."' )");
                        /* Завершение запроса */
                       // $result_query_insertAttachments->close();
            ...
            /*Конец логики по прикреплению файлов*/



            $_SESSION["success_messages"] = "<p class='success_message'>Обращение №".$CurrentInsertRequestID."создано успешно <br /><br />Обращение создано</p>";
